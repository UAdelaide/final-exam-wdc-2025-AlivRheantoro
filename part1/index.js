  ...
  if (usersCount[0].count === 0) {
    ...
    await db.query(`
      INSERT INTO WalkRequests (dog_id, requested_time, duration_minutes, location, status) VALUES
        ...
        ((SELECT dog_id FROM Dogs WHERE name = 'Dug'), '2025-06-11 07:00:00', 60, 'Central Park', 'open'),
        ((SELECT dog_id FROM Dogs WHERE name = 'Egg'), '2025-06-12 18:00:00', 30, 'Riverside', 'completed'),
        ((SELECT dog_id FROM Dogs WHERE name = 'Buddy'), '2025-06-13 10:00:00', 40, 'Mountain View', 'cancelled')
    `);

    await db.query(`
      INSERT INTO WalkApplications (request_id, walker_id, status) VALUES
        (1, (SELECT user_id FROM Users WHERE username='bobwalker'), 'accepted'),
        (2, (SELECT user_id FROM Users WHERE username='eggwalker'), 'pending'),
        (4, (SELECT user_id FROM Users WHERE username='bobwalker'), 'accepted')
    `);

    await db.query(`
      INSERT INTO WalkRatings (request_id, walker_id, owner_id, rating, comments) VALUES
        (4, (SELECT user_id FROM Users WHERE username='bobwalker'), (SELECT user_id FROM Users WHERE username='eggwalker'), 5, 'Great walk!'),
        (1, (SELECT user_id FROM Users WHERE username='bobwalker'), (SELECT user_id FROM Users WHERE username='alice123'), 4, 'Good walker.')
    `);
  }
}

// Setup database and then start server
setupDatabase().then(() => {
  const PORT = 3000;
